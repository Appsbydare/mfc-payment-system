    ...
    try {
        ...
        if (format === 'csv') {
            const headers = [
                ...
                'Discount', 'Discount %', 'Verification Status', 'Invoice #', 'Invoiced Amount', 'Amount',
...
router.post('/rewrite-master', async (req, res) => {
    try {
        ...
        const dataObjects = sheetData.map((row) => ({
            ...
            'Number of Sessions': row['Number of Sessions'] || row['numberOfSessions'] || 0,
...
router.post('/upsert-master', async (req, res) => {
    try {
        ...
        const toSheetObject = (r) => ({
            ...
            'Number of Sessions': r.numberOfSessions ?? r['Number of Sessions'] ?? 0,
